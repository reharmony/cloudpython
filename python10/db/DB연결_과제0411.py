...
def db_process_insert(id, title, content, director,img):
    ...
    # 3. sql문 만들어서 -> 전송
    sql = "insert into movieinfo values ('" + id + "', '" + title + "', '" + content + "', '"+ director + "', '"+ img + "')"
    cur.execute(sql)
    print()
...
def db_process_select(id):
    ...
    # 3. sql문 만들어서 -> 전송
    sql = "select * from movieinfo where id = '" + id + "'"
    cur.execute(sql)
    print()
    print("3. sql문 만들어서 -> 전송 성공...")
...
def db_process_update(id, title, content, director, img):
    ...
    # 3. sql문 만들어서 -> 전송
    sql = "update movieinfo set title ='" + title + "', content ='"+ content +"', director='" + director + "', img='" + img + "' where id='" + id + "'"
    print(sql)
    cur.execute(sql)
    print()
    print("3. sql문 만들어서 -> 전송 성공...")
